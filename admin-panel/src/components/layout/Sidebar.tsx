...
const MENU_GROUPS = [
  ...
  },
  {
    label: 'OPERATIONS',
    items: [
      { name: 'Vehicle Services', href: '/services' },
      { name: 'Workshop Tickets', href: '/claims' },
      { name: 'Finance & Loans', href: '/loans' },
    ]
  },
  {
    label: 'MANAGEMENT',
  ...
  const filteredGroups = MENU_GROUPS.map(group => {
    ...
    if (role === 'EXECUTIVE') {
      ...
      if (group.label === 'OPERATIONS') {
        items = items.filter(i => ['Vehicle Services', 'Workshop Tickets', 'Finance & Loans'].includes(i.name))
      }
      items = items.filter(i => !['CRM', 'Reports'].includes(i.name))
    } else if (role === 'MANAGER') {
      // Strict role-based filtering for Managers - Allow Operations now!
      if (group.label === 'OPERATIONS') {
        items = items.filter(i => ['Vehicle Services', 'Workshop Tickets'].includes(i.name))
      }
      ...
      `}>
        <div className="p-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <img src="/logo.png" alt="Movish Auto" className="h-12 w-auto object-contain" />
          </div>
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all mb-0.5 ${
                      isActive 
                        ? 'bg-[#084D8C] text-white shadow-lg shadow-[#084D8C]/20' 
                        : 'text-gray-600 hover:bg-gray-50 hover:text-[#084D8C]'
                    }`}
                  >
                    {item.name}
                  </Link>
        ...
        <div className="p-4 border-t border-gray-100">
          <div className="px-4 py-3 mb-2">
            ...
            <p className="text-[10px] font-medium text-[#D8232A] uppercase">{role}</p>
          </div>
          <button 
            onClick={() => {
              localStorage.removeItem('movish_mock_session');
              localStorage.removeItem('movish_mock_email');
              supabase.auth.signOut().then(() => window.location.href = '/login')
            }}
            className="w-full flex items-center gap-3 px-4 py-3 text-sm font-semibold text-[#D8232A] hover:bg-red-50 rounded-xl transition-all"
          >
            Logout
          </button>
        </div>

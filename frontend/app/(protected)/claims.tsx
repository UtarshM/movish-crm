  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Workshop Tickets</Text>
      ...
      <FlatList data={items} keyExtractor={i => i.id} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={Colors.primary} />}
        ...
        ListEmptyComponent={<View style={styles.empty}><Ionicons name="construct-outline" size={48} color={Colors.textLight} /><Text style={styles.emptyText}>No tickets found</Text></View>}
        renderItem={({ item }) => {
          const sc = StatusColors[item.status] || StatusColors.filed;
          return (
            <View style={styles.card}>
              <View style={styles.cardTop}>
                <View style={{ flex: 1 }}><Text style={styles.cardName}>{item.customerName || item.customer_name}</Text><Text style={styles.cardMeta}>{(item.claimType || item.claim_type || 'Scheduled Service')?.replace(/_/g, ' ')}</Text></View>
